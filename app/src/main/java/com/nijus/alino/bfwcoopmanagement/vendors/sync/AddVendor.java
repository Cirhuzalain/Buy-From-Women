...
public class AddVendor extends IntentService {
    ...
    protected void onHandleIntent(@Nullable Intent intent) {
        ...
        if (intent != null) {
            ...
            GeneralVendor general = farmerData.getParcelable("general");
            ForecastVendor forecast = farmerData.getParcelable("forecast");
            DemographicVendor demographic = farmerData.getParcelable("demographic");
            BaseLineVendor baseLine = farmerData.getParcelable("baseline");
            FinanceVendor finance = farmerData.getParcelable("finance");
            ServiceAccessVendor serviceAccess = farmerData.getParcelable("serviceAccess");
            AccessToInformationVendor accessToInformation = farmerData.getParcelable("access_information");
            ...
            if (map != null) {
                ...
                ContentValues landVendor;
                double landInfo;

                for (Object landSize : map.keySet()) {
                    landVendor = new ContentValues();
                    landInfo = (double) map.get(landSize.toString());
                    landVendor.put(BfwContract.LandPlot.COLUMN_PLOT_SIZE, landInfo);
                    landVendor.put(BfwContract.LandPlot.COLUMN_FARMER_ID, id);
                    landVendor.put(BfwContract.LandPlot.COLUMN_LAND_ID, UUID.randomUUID().toString());
                    landVendor.put(BfwContract.LandPlot.COLUMN_IS_SYNC, 0);
                    landVendor.put(BfwContract.LandPlot.COLUMN_SERVER_ID, 0);
                    landVendor.put(BfwContract.LandPlot.COLUMN_IS_UPDATE, 0);

                    getContentResolver().insert(BfwContract.LandPlot.CONTENT_URI, landVendor);
                }
            }
